...
public static class SimpleEdits
{
    ...
    public static void SetSuggestedMemories(this PKM pk)
    {
        switch (pk)
        {
            case PA9 pa9 when !pk.IsUntraded:
                pa9.ClearMemoriesHT();
                break;
    ...
    public static void SetHandlerAndMemory(this PKM pk, ITrainerInfo trainer, IEncounterTemplate enc)
    {
        ...
        var tradeEvo = pk.Species > enc.Species && pk.TradeEvolutionRequired(enc);
        var expect = trainer.IsFromTrainer(pk) ? 0 : 1;

        if (!tradeEvo && pk.CurrentHandler == expect && expect == 0)
            return;
    ...
    /// <summary>
    /// Determines if a PKM requires Trading to Evolve
    /// </summary>
    /// <param name="pk">PKM to modify</param>
    /// <param name="enc">Encounter template originated from</param>
    public static bool TradeEvolutionRequired(this PKM pk, IEncounterTemplate enc)
    {
